...
class Winda:
    ...
    def znajdz_najblizsze_zgloszenie(self):
        def najblizsze(zgloszenia, aktualne_pietro, atrybut):
            return min(zgloszenia, key=lambda x: abs(aktualne_pietro - getattr(x, atrybut)), default=None)

        najblizsze_zgloszenie_a = najblizsze(self.pasazerowie_w_windzie, self.pietro, 'cel')
        najblizsze_zgloszenie_b = najblizsze(self.zgloszenia, self.pietro, 'start')

        if najblizsze_zgloszenie_a and najblizsze_zgloszenie_b:
            return (najblizsze_zgloszenie_a.cel if 
                    abs(self.pietro - najblizsze_zgloszenie_a.cel) < abs(self.pietro - najblizsze_zgloszenie_b.start) 
                    else najblizsze_zgloszenie_b.start)
        elif najblizsze_zgloszenie_a:
            return najblizsze_zgloszenie_a.cel
        elif najblizsze_zgloszenie_b:
